  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      for (let keepAlive = 0; keepAlive < 3; keepAlive += 1) {
        ...
        yield* sendRuntimeFrame(fullReplay.connection, RuntimeClientFrame.cases.KeepAlive.make({}));
        expect((yield* nextRuntimeFrame(fullReplay.connection))._tag).toBe("KeepAliveAck");
      ...
      let hibernatedBootId = fullReplay.probe.runtime.bootId;
      for (let frameIndex = 0; frameIndex < 32; frameIndex += 1) {
        const frame = yield* nextRuntimeFrame(fullReplay.connection);
        if (frame._tag === "Pong" && frame.nonce === hibernationNonce) {
          hibernatedBootId = frame.bootId;
          break;
        }
      }
      expect(hibernatedBootId).not.toBe(fullReplay.probe.runtime.bootId);
      yield* Effect.logInfo("Hibernation wake evidence", {
        beforeBootId: fullReplay.probe.runtime.bootId,
      ...
      expect(routed.probe.connectionId).not.toBe(fullReplay.probe.connectionId);
