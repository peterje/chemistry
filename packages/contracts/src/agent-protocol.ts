...
/** A bounded title shown for one durable conversation. */
export const ChatTitle = Schema.NonEmptyString.check(Schema.isMaxLength(80));

/** A durable conversation entry rendered by the navigation sidebar. */
export const ChatSummary = Schema.Struct({
  sessionId: SessionId,
  title: ChatTitle,
  createdAt: Schema.Number,
  updatedAt: Schema.Number,
}).annotate({ identifier: "ChatSummary" });

/** A decoded durable conversation summary. */
export interface ChatSummary extends Schema.Schema.Type<typeof ChatSummary> {}

/** Recently active durable conversations in descending activity order. */
export const ChatList = Schema.Struct({
  chats: Schema.Array(ChatSummary),
}).annotate({ identifier: "ChatList" });

/** A decoded durable conversation list. */
export interface ChatList extends Schema.Schema.Type<typeof ChatList> {}
...
const createChat = Rpc.make("createChat", {
  payload: { sessionId: SessionId },
  success: ChatSummary,
  error: AgentRpcError,
});

const listChats = Rpc.make("listChats", {
  payload: {},
  success: ChatList,
  error: AgentRpcError,
});

const touchChat = Rpc.make("touchChat", {
  payload: {
    sessionId: SessionId,
    prompt: Schema.NonEmptyString.check(Schema.isMaxLength(32_000)),
  },
...
/** Session-scoped Effect RPC contract served by each per-chat Durable Object. */
export class AgentSessionRpcs extends RpcGroup.make(
  getSession,
  getRuntime,
  updateContext,
  sendMessage,
...
/** Internal Effect RPC contract served by the singleton chat-catalog Durable Object. */
export class ChatCatalogRpcs extends RpcGroup.make(createChat, listChats, touchChat) {}

/** Public Effect RPC contract served to the browser by the backend Worker. */
