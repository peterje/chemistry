...
import { DurableExecution } from "@chemistry/agent-runtime/durable-execution";
import {
  acceptReplayBacklog,
  beginReplayHandoff,
  drainReplayPending,
...
import { RuntimeIdSource } from "@chemistry/agent-runtime/runtime-id-source";
...
/** Server-persisted conversation activity required by accepted WebSocket turns. */
export class ChatActivityCatalog extends Context.Service<
  ChatActivityCatalog,
  {
    readonly touch: (
      sessionId: SessionId,
      prompt: string,
    ) => Effect.Effect<void, AgentPersistenceError>;
  }
>()("@chemistry/ChatActivityCatalog") {}
...
const buildRuntimeWebSocketAdapter = Effect.fn("RuntimeWebSocketAdapter.make")(function* () {
  ...
  const chatActivity = yield* ChatActivityCatalog;
    ...
    switch (frame._tag) {
      ...
      case "SubmitTurn": {
        ...
        if (!admission.duplicate) {
          const recorded = yield* chatActivity
            .touch(attachment.sessionId, frame.prompt)
            .pipe(Effect.result);
